<div class="p-6">
                        ...
                        @forelse ($semesters as $semester)
                            <tr>
                                ...
                                <x-td-table>
                                    ...
                                    <flux:button size="xs" wire:click="delete({{ $semester->id }})" color="red" variant="primary">Eliminar</flux:button>
                                </x-td-table>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center">No hay semestres</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if($isOpen)
        <flux:modal wire:model.self="isOpen" class="md:w-96">
            <div class="space-y-6">
                <flux:heading size="lg">{{ $semesterId ? 'Editar Semestre' : 'Crear Semestre' }}</flux:heading>
                ...
                <div class="flex gap-2">
                    ...
                    <flux:button wire:click="save" variant="primary">{{ $semesterId ? 'Actualizar' : 'Guardar' }}</flux:button>
                </div>
            </div>
        </flux:modal>
    @endif
