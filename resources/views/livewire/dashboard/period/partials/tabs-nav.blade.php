        ...
        @foreach($tabs as $route => $data)
            <a wire:navigate
               ...
               style="{{ request()->routeIs($route)
                ? 'background-color: var(--color-primary); color: var(--color-bg); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.3);'
                : 'color: var(--color-secondary);' }}"
               onmouseover="{{ request()->routeIs($route) ? '' : "this.style.color='var(--color-primary-2)'; this.style.backgroundColor='var(--color-icon-bg)';" }}"
               onmouseout="{{ request()->routeIs($route) ? '' : "this.style.color='var(--color-secondary)'; this.style.backgroundColor='transparent';" }}">
