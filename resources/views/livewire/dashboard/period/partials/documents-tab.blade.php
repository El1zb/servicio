...
{{-- Botón principal (escritorio): mismo diseño que "Nuevo X" en el resto de
     la app, en el topbar. En mobile, el respaldo solo-ícono vive a la
     izquierda del switcher — ver partials/tabs-nav.blade.php. --}}
@push('topbar-actions')
...
<div class="space-y-6">
    ...
    @if($paginatedFiles->count())
        <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @foreach($paginatedFiles as $file)
                <div class="period-card group cursor-pointer"
                    x-data="{ pressTimer: null, longPressed: false }"
                    @mousedown="pressTimer = setTimeout(() => { longPressed = true; $wire.openDocViewer({{ $file->id }}) }, 500)"
                    @mouseup="clearTimeout(pressTimer)"
                    @mouseleave="clearTimeout(pressTimer)"
                    @touchstart.passive="pressTimer = setTimeout(() => { longPressed = true; $wire.openDocViewer({{ $file->id }}) }, 500)"
                    @touchend="clearTimeout(pressTimer)"
                    @contextmenu.prevent
                    @click="if (longPressed) { longPressed = false } else { $wire.editDocument({{ $file->id }}) }">
                    ...
                    <div class="flex items-center justify-end gap-1">
                        @if($file->file_path)
                            <button wire:click.stop="openDocViewer({{ $file->id }}, '{{ $file->file_path }}')"
                                    @mousedown.stop @touchstart.stop.passive
                                    title="Ver Word"
                        ...
                        @if($file->example_path)
                            <button wire:click.stop="openDocViewer({{ $file->id }}, '{{ $file->example_path }}')"
                                    @mousedown.stop @touchstart.stop.passive
                                    title="Ver PDF"
                                    class="w-7 h-7 flex items-center justify-center rounded-full flex-shrink-0
                                           bg-transparent cursor-pointer
    ...
    @include('livewire.dashboard.period.modals.document-viewer')
