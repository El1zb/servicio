<div class="p-6">
    <h1 class="text-2xl font-bold mb-4">Campus</h1>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        @forelse ($campuses as $campus)
                            <tr>
                                <x-td-table>{{ $campus->id }}</x-td-table>
                                <x-td-table>{{ $campus->name }}</x-td-table>
                                <x-td-table>
                                    <flux:button size="xs" wire:click="edit({{ $campus->id }})" color="emerald" variant="primary">Editar</flux:button>
                                    <flux:button size="xs" wire:click="delete({{ $campus->id }})" color="red" variant="primary">Eliminar</flux:button>
                                </x-td-table>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center">No hay campus</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <flux:modal wire:model.self="isOpen" class="md:w-96">
        <div class="space-y-6">
            <flux:heading size="lg">{{ $campusId ? 'Editar Campus' : 'Crear Campus' }}</flux:heading>
