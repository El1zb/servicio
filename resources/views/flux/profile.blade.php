...
$iconClasses = Flux::classes('text-(--sidebar-subtitle-text) dark:text-(--sidebar-subtitle-text) group-hover:text-(--sidebar-item-text-hover) dark:group-hover:text-(--sidebar-item-text-hover)')
...
$classes = Flux::classes()
    ...
    ->add('p-1 hover:bg-(--sidebar-item-bg-hover)')
    ;
@endphp
...
<button type="button" {{ $attributes->class($classes) }} data-flux-profile>
    ...
    <?php if ($name): ?>
        <span class="mx-2 text-sm text-(--sidebar-subtitle-text) dark:text-(--sidebar-subtitle-text) group-hover:text-(--sidebar-item-text-hover) dark:group-hover:text-(--sidebar-item-text-hover) font-medium truncate">
            {{ $name }}
        </span>
    <?php endif; ?>
